...
describe('stage three design-system and page-boundary contract', () => {
  ...
  it('freezes stage-three visual ownership and page-scope rules', () => {
    ...
    expect(detailInfo).toContain('查看与下载')
    expect(detailInfo).toContain('编辑与管理')
    expect(homePage).toContain(':visible="selectedImageUuids.length > 0"')
    expect(profilePage).toContain('累计创作')
    expect(profilePage).toContain('Number(user.value.publicLikeCount || 0)')
    expect(profilePage).not.toContain(".filter(img => img.visibility === 'PUBLIC')")
    expect(profileWorks).toContain('PUBLIC WORKS')
    expect(profileWorks).toContain('创作作品')
    expect(profileWorks).toContain('还没有分享过创作')
