<template>
  <aside class="square-side" aria-label="公开广场发现">
    <section class="side-section">
      <div class="side-title"><span>当前筛选</span><strong>{{ activeSortLabel }}</strong></div>
    ...
    <section class="side-section">
      ...
      <div v-if="activeCreators.length" class="creator-list">
        <button v-for="creator in activeCreators" :key="creator.username" type="button" class="creator-row" @click="emit('select-creator', creator.uuidOrId)">
          <span class="creator-avatar" :style="{ '--avatar-color': creator.avatarColor }">{{ creator.initial }}</span>
          <span><strong>{{ creator.displayName || creator.username }}</strong><small>本页 {{ creator.likeCount }} 次喜欢</small></span>
        </button>
      </div>
      <p v-else class="side-empty">暂无作者信息。</p>
    </section>
    <section v-if="featuredImage" class="side-section inspector-section">
      <div class="side-title"><span>推荐</span><strong>{{ featuredImage.categoryName || '未分类' }}</strong></div>
...
.tag-pill { min-height: 36px; padding: 0 var(--space-3); border: 1px solid var(--color-border-subtle); border-radius: var(--radius-round); background: transparent; color: var(--color-text-secondary); cursor: pointer; }
...
.creator-row strong,.creator-row small { display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
