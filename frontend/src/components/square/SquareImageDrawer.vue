<template>
  <Teleport to="body">
    <transition name="drawer-slide">
      <aside v-if="visible && image" class="image-drawer" aria-label="图片快速详情" tabindex="-1" @keydown.esc.prevent.stop="emit('close')">
...
<style scoped>
.image-drawer { position: fixed; z-index: var(--layer-drawer); top: calc(var(--nav-height) + var(--space-4)); right: var(--space-4); bottom: var(--space-4); width: min(var(--panel-drawer-width),calc(100vw - (2 * var(--space-4)))); max-width: 100%; overflow: auto; border: 1px solid var(--color-border-strong); background: var(--color-surface-1); box-shadow: var(--shadow-float); }
...
.drawer-slide-enter-active,.drawer-slide-leave-active{transition:transform var(--duration-overlay) var(--ease-standard),opacity var(--duration-overlay) var(--ease-standard)}
.drawer-slide-enter-from,.drawer-slide-leave-to{transform:translateX(28px);opacity:0}
