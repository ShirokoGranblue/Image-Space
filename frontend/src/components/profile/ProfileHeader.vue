...
.avatar-column{display:flex;min-width:0;flex-direction:column;align-items:flex-start;gap:var(--space-4)}.avatar-wrap{position:relative;width:122px;height:122px;flex:0 0 122px}.avatar{display:block;width:122px;height:122px;border:4px solid var(--color-surface-1);box-shadow:var(--shadow-float)}.avatar-upload{position:absolute;right:0;bottom:4px;display:grid;width:44px;height:44px;place-items:center;border:1px solid var(--color-border-strong);border-radius:50%;background:var(--color-surface-1);color:var(--color-text-primary);cursor:pointer;opacity:.64;transition:opacity var(--duration-fast) var(--ease-standard)}.avatar-upload--active{opacity:1}
.profile-stats{display:grid;width:100%;grid-template-columns:repeat(2,minmax(0,1fr));gap:var(--space-4)}.stat-item{display:flex;min-width:0;flex-direction:column;gap:var(--space-1);padding:var(--space-2) 0}.stat-item strong{display:block;min-width:3ch;font-family:var(--font-title);font-size:var(--text-xl);font-variant-numeric:tabular-nums}.stat-item span{color:var(--color-text-muted);font-size:var(--text-xs)}
