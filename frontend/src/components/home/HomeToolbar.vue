<template>
  <div class="home-toolbar">
    <header class="main-head"><div class="headline-block"><span class="eyebrow">PERSONAL IMAGES</span><h1>这里放着值得再次回看的图片。</h1><p>上传新的内容，或按分类、标签和可见范围继续整理已有图片。</p></div><div class="stats" aria-label="图片概览"><div class="stat"><strong>{{ total }}</strong><span>全部图片</span></div><div class="stat"><strong>{{ categoryCount }}</strong><span>分类</span></div><div class="stat"><strong>{{ selectedCount }}</strong><span>已选择</span></div></div></header>
    ...
    <section class="filter-strip" aria-label="快速筛选"><button v-for="option in visibilityOptions" :key="option.value" class="filter-chip" :class="{active:visibility===option.value}" type="button" @click="emit('select-visibility',option.value)">{{ option.label }}</button><Transition name="destructive-chip"><button v-if="selectedCount>0" class="filter-chip danger" type="button" @click="emit('batch-delete')">删除 {{ selectedCount }}</button></Transition></section>
  </div>
</template>
<script setup>
...
.eyebrow{display:inline-block;margin-bottom:var(--space-4);padding-left:var(--space-3);border-left:2px solid var(--color-vermilion);color:var(--color-text-secondary);font-size:var(--text-xs);font-weight:600;letter-spacing:.12em}
...
.stats{display:flex;align-items:stretch}.stat{display:flex;min-width:88px;flex-direction:column;align-items:center;justify-content:center;padding:0 var(--space-4);border-left:1px solid var(--color-border-subtle);text-align:center}.stat strong{font-family:var(--font-title);font-size:var(--text-2xl);font-weight:500}.stat span{color:var(--color-text-muted);font-size:var(--text-xs)}
...
.select-all,.primary-command,.filter-chip{min-height:var(--control-height-md);padding:0 var(--space-3);border:1px solid var(--color-border-subtle);border-radius:var(--radius-sm);background:var(--color-surface-1);color:var(--color-text-primary);cursor:pointer;transition:transform var(--duration-fast) var(--ease-out),background-color var(--duration-fast) ease,border-color var(--duration-fast) ease,color var(--duration-fast) ease}.select-all:active,.primary-command:active,.filter-chip:active{transform:scale(0.97)}.select-all.active,.filter-chip.active{border-color:var(--color-night);background:var(--color-night);color:var(--color-text-inverse)}.primary-command{display:inline-flex;align-items:center;justify-content:center;gap:var(--space-2);border-color:var(--color-vermilion);background:var(--color-vermilion);color:var(--color-text-inverse)}
.filter-strip{display:flex;flex-wrap:wrap;gap:var(--space-2);padding:var(--space-4) var(--space-3);border:1px solid var(--color-border-subtle);border-top:0}.filter-chip.danger{border-color:var(--color-error);color:var(--color-error)}
.destructive-chip-enter-active,.destructive-chip-leave-active{transition:opacity var(--duration-fast) var(--ease-out),transform var(--duration-fast) var(--ease-out)}
.destructive-chip-enter-from,.destructive-chip-leave-to{opacity:0;transform:scale(0.97)}
...
@media(prefers-reduced-motion:reduce){.select-all:active,.primary-command:active,.filter-chip:active{transform:none}.destructive-chip-enter-active,.destructive-chip-leave-active{transition:opacity 200ms ease}.destructive-chip-enter-from,.destructive-chip-leave-to{opacity:0;transform:none}}
</style>
